        ...
        personalRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    ...
                    float entRatio;
                    if(snapshot.hasChild("monthEnRatio")){
                        entRatio=Integer.parseInt(snapshot.child("monthEnRatio").getValue().toString());
                    }else {
                        entRatio=0;
                    }
                    ...
                    Toast.makeText(MonthlyAnalyticActivity.this,"setStatusAndImageResource Errors",Toast.LENGTH_SHORT).show();
                }
            }
